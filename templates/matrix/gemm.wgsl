...
fn main([[builtin(global_invocation_id)]] global_id: vec3<u32>) {
    ...
    for(var k: u32 = 0u; k < {{ i_dims[0][1] / 4 | int }}u; k = k + 1u) {
        ...
        let mat_left = mat4x4<f32>(
                              ...
                              {{ inputs[0] }}.data[index_left + {{ i_dims[0][1] / 4 | int }}u],
                              {{ inputs[0] }}.data[index_left + {{ 2 * i_dims[0][1] / 4 | int }}u],
                              {{ inputs[0] }}.data[index_left + {{ 3 * i_dims[0][1] / 4 | int }}u],
                          );
          
        let mat_right = mat4x4<f32>(
                              {{ inputs[1] }}.data[index_right], 
                              {{ inputs[1] }}.data[index_right + {{ i_dims[1][1] / 4 | int }}u],
                              {{ inputs[1] }}.data[index_right + {{ 2 * i_dims[1][1] / 4 | int }}u],
                              {{ inputs[1] }}.data[index_right + {{ 3 * i_dims[1][1] / 4 | int }}u],
                          );
...
{%- if inputs | length == 3 -%}
    ...
    for(var index_mat: u32 = 0u; index_mat < 4u; index_mat = index_mat + 1u) {
            {{ outputs[0] }}.data[index + index_mat * {{ i_dims[1][1] / 4 | int }}u] = {%- if alpha != 1 -%} {{ alpha | float }} * {%- endif -%}tmpsum[index_mat] + {%- if beta != 1 -%} {{ beta | float }} * {%- endif -%}bias[index_mat];
    }       
{%- else -%}
    for(var index_mat: u32 = 0u; index_mat < 4u; index_mat = index_mat + 1u) {
            {{ outputs[0] }}.data[index + index_mat * {{ i_dims[1][1] / 4 | int }}u] = {%- if alpha != 1 -%} {{ alpha | float }} * {%- endif -%}tmpsum[index_mat];
    }       
{%- endif -%}  
}