...
fn main([[builtin(global_invocation_id)]] global_id: vec3<u32>) {
    let y = global_id.x % {{ right_columns / 4 }}u;
    let x = global_id.x / {{ right_columns / 4 }}u;
    ...
    for(var k: u32 = 0u; k < {{ left_columns / 4 }}u; k = k + 1u) {
        let index_left = x * {{ left_columns }}u + k; 
        let index_right = k * {{ left_columns }}u + y; 

        let mat_left = mat4x4<f32>(
                              {{ input[0] }}.data[index_left], 
                              {{ input[0] }}.data[index_left + {{ left_columns / 4 }}u],
                              {{ input[0] }}.data[index_left + 2u * {{ left_columns / 4 }}u],
                              {{ input[0] }}.data[index_left + 3u * {{ left_columns / 4 }}u],
                          );
          
        let mat_right = mat4x4<f32>(
                              {{ input[1] }}.data[index_right], 
                              {{ input[1] }}.data[index_right + {{ right_columns / 4 }}u],
                              {{ input[1] }}.data[index_right + 2u * {{ right_columns / 4 }}u],
                              {{ input[1] }}.data[index_right + 3u * {{ right_columns / 4 }}u],
                          );
...
{% if input | length == 3 %}
    ...
    for(var index_mat: u32 = 0u; index_mat < 4u; index_mat = index_mat + 1u) {
            {{ output[0] }}.data[index + index_mat * {{ right_columns / 4 }}u] = {% if alpha != 1 %} {{ alpha | float }} * {% endif %}tmpsum[index_mat] + {% if beta != 1 %} {{ beta | float }} * {% endif %}bias[index_mat];
    }       
{% else %}
    for(var index_mat: u32 = 0u; index_mat < 4u; index_mat = index_mat + 1u) {
            {{ output[0] }}.data[index + index_mat * {{ right_columns / 4 }}u] = {% if alpha != 1 %} {{ alpha | float }} * {% endif %}tmpsum[index_mat];
    }       
{% endif %}  
}
{% endblock main %}