...
fn main([[builtin(global_invocation_id)]] global_id: vec3<u32>) {
    ...
    for(var k: u32 = 0u; k < {{ left_columns / 4 | int }}u; k = k + 1u) {
        ...
        let mat_left = mat4x4<f32>(
                              ...
                              var_{{ inputs[0] }}.data[index_left + {{ left_columns / 4| int }}u],
                              var_{{ inputs[0] }}.data[index_left + {{ 2 * left_columns / 4 | int }}u],
                              var_{{ inputs[0] }}.data[index_left + {{ 3 * left_columns / 4 | int }}u],
                          );
          
        let mat_right = mat4x4<f32>(
                              var_{{ inputs[1] }}.data[index_right], 
                              var_{{ inputs[1] }}.data[index_right + {{ right_columns / 4 | int }}u],
                              var_{{ inputs[1] }}.data[index_right + {{ 2 * right_columns / 4 | int }}u],
                              var_{{ inputs[1] }}.data[index_right + {{ 3 * right_columns / 4 | int }}u],
                          );
...
{%- if inputs | length == 3 -%}
    ...
    for(var index_mat: u32 = 0u; index_mat < 4u; index_mat = index_mat + 1u) {
            var_{{ outputs[0] }}.data[index + index_mat * {{ right_columns / 4 | int }}u] = {%- if alpha != 1 -%} {{ alpha | float }} * {%- endif -%}tmpsum[index_mat] + {%- if beta != 1 -%} {{ beta | float }} * {%- endif -%}bias[index_mat];
    }       
{%- else -%}
    for(var index_mat: u32 = 0u; index_mat < 4u; index_mat = index_mat + 1u) {
            var_{{ outputs[0] }}.data[index + index_mat * {{ right_columns / 4 | int }}u] = {%- if alpha != 1 -%} {{ alpha | float }} * {%- endif -%}tmpsum[index_mat];
    }       
{%- endif -%}  
}