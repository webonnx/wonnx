...
fn main([[builtin(global_invocation_id)]] global_id: vec3<u32>) {
        ...
        let tmpMat_{{ input[0] }} = transpose(mat4x4<f32>({{ input[0] }}.data[index], 
                            ...
                            {{ input[0] }}.data[index + {{ 2 * len_1 }}u],
                            {{ input[0] }}.data[index + {{ 3 * len_1 }}u],
                        ));

        let index = y * {{ len_0 }}u + x;

        {{ output[0] }}.data[index] = tmpMat_{{ input[0] }}[0u];
        {{ output[0] }}.data[index + {{ len_0 / 4 | int }}u] = tmpMat_{{ input[0] }}[1u];
        {{ output[0] }}.data[index + {{ 2 * len_0 / 4 | int }}u] = tmpMat_{{ input[0] }}[2u];
        {{ output[0] }}.data[index + {{ 3 * len_0 / 4 | int }}u] = tmpMat_{{ input[0] }}[3u];
}
{% endblock main %}