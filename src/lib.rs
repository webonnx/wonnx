...
async fn execute_gpu() -> Option<Vec<f32>> {
    ...
    //   The destination of a copy.
    //   The source of a copy.
    let a = wgpu::BindGroupEntry {
        binding: 0,
        resource: storage_buffer.as_entire_binding(),
    };

    let (compute_pipeline, bind_group) = compute::unit_compute(&device, &[a], "cos");
...
fn main() {
    #[cfg(not(target_arch = "wasm32"))]
    {
        ...
        // let mut array = Array::range(0., LEN as f32, 1.0);
        // array.iter_mut().for_each(|x| {
        //     *x = f32::cos(*x) + f32::sin(*x) + f32::tan(*x);
        // });
        // array.iter_mut().for_each(|x| {
        //     *x = f32::cos(*x) + f32::sin(*x) + f32::tan(*x);
        // });
        // println!("arr[0..5]: {:#?}", &array[2]);
    }
    #[cfg(target_arch = "wasm32")]
    {
