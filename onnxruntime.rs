...
fn run() -> Result<(), Error> {
    ...
    let mut session = environment
        ...
        .with_model_from_file("tests/mnist-8.onnx")?;
    ...
    let n: u32 = session.inputs[0]
        ...
        .map(|d| d.unwrap())
        .product();
    let array = Array::linspace(0.0_f32, 28.0 * 28.0, n as usize)
        .into_shape(input0_shape)
        .unwrap();
    let input_tensor_values = vec![array];

    let time_pre_compute = Instant::now();
    let outputs: Vec<OrtOwnedTensor<f32, _>> = wonnx::run(&mut session, input_tensor_values)?;
